...
if (len(argv) > 1):
    if (argv[1] == '-h' or argv[1] == '--help'):
        print('Usage: Psi_freehyp.py [R_start [phase_over_pi]]       ; provide R_start in Angstrom')
        exit()
...
with open("psi_freehyp/norm_psi_freehyp_{}_{}.out".format(R_start,phase_over_pi), "w") as f:
    with open("psi_freehyp/imag_norm_psi_freehyp_{}_{}.out".format(R_start,phase_over_pi), "w") as g:
        with open("psi_freehyp/real_norm_psi_freehyp_{}_{}.out".format(R_start,phase_over_pi), "w") as h:
            ...
            h.write("# R / Angstrom     real(psi_freehyp(R_au,Va={},Vb={},red_mass={},R_start_au=sc.angstrom_to_bohr({}),phase=pi*{}))\n".format(Va, Vb, red_mass, R_start, phase_over_pi))
             
            for R in np.arange(1.5,10.,0.0001):
                print("R / Angstrom = ", R)
                psi = wf.psi_freehyp(sc.angstrom_to_bohr(R),Va,Vb,red_mass,R_start_au,phase)
                 
                f.write(str(R) + " " + str(        psi ) + "\n")
                g.write(str(R) + " " + str(np.imag(psi)) + "\n")
                h.write(str(R) + " " + str(np.real(psi)) + "\n")

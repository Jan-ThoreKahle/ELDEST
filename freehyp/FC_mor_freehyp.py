...
# Set Morse potential curve - provide alpha and Req in a.u. (inverse Bohr, Bohr), De in eV
alpha = 1.5
Req = 6.
De = 0.003
#alpha = 15.3994
...
mode = "s"
outfile = "fc-gs-freehyp-scan.out"
...
if (mode == "R"):
    with open(outfile, "w") as f:
        f.write("#S = wf.FCmor_freehyp(n,alpha_au,Req_au,De_au,red_mass,Va,Vb,R_start_au,R_min,R_max,phase,limit)\n")
        f.write(f"#  = wf.FCmor_freehyp({n},{alpha},{Req},{De_au},{red_mass},{Va},{Vb},R_start_au,{R_min},{R_max},phase={phi},limit=5000)\n")
        f.write(f"#R_start / A  {'S':<55}  |S|**2\n")

        for R in np.arange(0.0010, 0.0300, 0.0001):
            S = wf.FCmor_freehyp(n,alpha,Req,De_au,red_mass,Va,Vb,sc.angstrom_to_bohr(R),R_min,R_max,phase=phi,limit=5000)
            f.write(f"{R:<11.11}   {S:< 20.20E}  {np.abs(S)**2:.20E}\n")
    ...
    with open(outfile, "w") as f:
        ...
        f.write(f"#  = wf.FCmor_freehyp({n},{alpha},{Req},{De_au},{red_mass},{Va},{Vb},{R_start_au},{R_min},{R_max},phase=phase_over_pi*pi,limit=5000)\n")
        f.write(f"#phase_over_pi  {'S':<55}  |S|**2\n")

        for p in np.arange(0., 2., 0.01):
            S = wf.FCmor_freehyp(n,alpha,Req,De_au,red_mass,Va,Vb,R_start_au,R_min,R_max,phase=p*np.pi,limit=5000)
            f.write(f"{p:<14.14}  {S:< 20.20E}  {np.abs(S)**2:.20E}\n")
else:
    S = wf.FCmor_freehyp(n,alpha,Req,De_au,red_mass,Va,Vb,R_start_au,R_min,R_max,phase=phi,limit=5000)
    print(f"R_start / A  phase_over_pi  n  {'S':<55} |S|**2")
    print(f"{R_start:<11.11}  {phase_over_pi:<14.14} {n} {S:< 20.20E}  {np.abs(S)**2:.20E}")

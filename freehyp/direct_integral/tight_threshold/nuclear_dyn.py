...
print('n_gs  ' +'n_fin  ' + '<fin|gs>                             ' + '|<fin|gs>|**2')
outfile.write('n_gs  ' +'n_fin  ' + '<fin|gs>                             ' + '|<fin|gs>|**2\n')
for i in range (0,n_gs_max+1):
    tmp = []
    for j in range (0,n_fin_max+1):
        if (fin_pot_type == 'morse'):
            FC = wf.FC(j,fin_a,fin_Req,fin_de,red_mass,
                i,gs_a,gs_Req,gs_de,R_min,R_max)
            tmp.append(FC)
            outfile.write('{:4d}  {:5d}  {:14.10E}  {:14.10E}\n'.format(i,j,FC,np.abs(FC)**2))
            print('{:4d}  {:5d}  {:14.10E}  {}'.format(i,j,FC,np.abs(FC)**2))
...
#
        if (fin_pot_type == 'hyperbel'):
            ...
            nmu = 0
            while ((nmu <= n_fin_max_list[-1]) or (np.abs(dir_J1) >= threshold)) and (nmu <= n_fin_max_list[-1] + 100): # Cap sum if n_fin > max n_fin_max & J_dir fell sub-threshold or after 100 additional terms
                if (nmu > n_fin_max_list[-1]):           # If precalced list of gs-fin-FCF is exhausted but still J_dir >= threshold:
                    ...
                    for i in range (0,n_gs_max+1):       # b) calc new gs-fin-FCF (one for each gs vibr state kappa)
                        ...
                        outfile.write(f'{nmu} {E_mus[-1]} {gs_fin[0][-1]}\n') #?
 ...
 #       
                square = np.absolute(J + dir_J1)**2     # |J_mu|**2
                print(nmu, dir_J1, square)
                outfile.write(f'{nmu} {dir_J1} {square}\n')
                sum_square = sum_square + square        # |J|**2 = sum_mu |J_mu|**2
                nmu = nmu + 1

...
while (De_res < De_max):
    ...
    while (is_correct_nmax(lambda_param(mu,De_res,res_alpha),nmax_res)):
        ...
        #    fin_alpha = alpha_min(mu,De_fin,nmax_fin) + alpha_step
        #    while (is_correct_nmax(lambda_param(mu,De_fin,fin_alpha),nmax_fin)):
        #        # consider only cases, where the overlap of both res vib states with the
        #        # ground state is sufficient
        #        FCfin1 = wf.FC(1,res_alpha,Req,De_res,mu,
        #                     0,fin_alpha,Req,De_fin,R_min,R_max)
        #        FCfin0 = wf.FC(0,res_alpha,Req,De_res,mu,
        #                     0,fin_a,Req,De_fin,R_min,R_max)

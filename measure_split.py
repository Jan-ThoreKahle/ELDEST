...
#direct ionization
#fun_t_dir_1 = lambda t1: FX_t1(t1)   * np.exp(1j * (E_fin_laser_au + TK_laser_au) * t1) \
#                                     * np.exp(1j * E_kin_au * (t1-t_au)) \
#                                     * np.exp(-1j * (TK_laser_au + E_fin_laser_au) * t_au)
...
if (integ == 'romberg'):
    ...
    res_inner = lambda t1: (
                              np.exp(-t_au * 1j* (Er_au + T_K
                                                  - 1j* np.pi * (VEr_au**2)))
                            * np.exp(t1 * 1j* (Er_au + T_K
                                                  - 1j* np.pi * (VEr_au**2)))
                            - np.exp(-1j* t_au * (E_kin_au + E_fin_au + T_K))
                            * np.exp(1j* t1 * (E_kin_au + E_fin_au + T_K))
                           )
...
part1_integral = lambda t1: FX_t1(t1) \
                            * np.exp(Er_au - 1j*np.pi*VEr_au**2 + T_K)

def part1const(Er_au, VEr_au, E_kin_au, E_fin_au, t_au):
    exp = np.exp(-1j * t_au * (Er_au - 1j*np.pi*VEr_au**2 - E_kin_au - E_fin_au))
    oneover = -1./(Er_au - 1j*np.pi*VEr_au**2 - E_kin_au - E_fin_au)
    part1 = exp * oneover
    return part1
...
lower_E_min = sciconv.ev_to_hartree(0.0)
lower_E_max = sciconv.ev_to_hartree(0.0)
upper_E_min = sciconv.ev_to_hartree(0.0)
...
while (E_kin_au <= E_max_au):
    if (E_kin_au >= lower_E_min and E_kin_au <= lower_E_max):
        ...
        Ekins2.append(sciconv.hartree_to_ev(E_kin_au))
    if (E_kin_au >= lower_E_min and E_kin_au <= lower_E_max and lower_E_max - upper_E_min < 1E-7):
        Ekins1.append(sciconv.hartree_to_ev(E_kin_au))
...
# integral 1
            ...
            for time in range(0,n+1):
                #J = J + (res_tuples[len(Ekins1)*time][1]
...
#-------------------------------------------------------------------------
    ...
    R0 = R
    v0 = a * timestep_au + v0
    print "R0 = ", sciconv.bohr_to_angstrom(R0)
...
# integral 1
            if (integ_outer == "quadrature"):
                ...
                #                    (Er_au - 1j * VEr_au**2 - E_kin_au - E_fin_au))
                #          )
                #print "exp_res**2 = ", np.absolute(exp_res)**2
                #print "I1**2 = ", np.absolute(I1[0])**2
            ...
            J = dir_J1
            #print "dir_J1 = ", dir_J1
            #print "res_J1 = ", np.absolute(res_J1)**2
            #print "indir_J1 = ", np.absolute(indir_J1)**2
            J = 0
            square = 0
            for time in range(0,n+1):
                #J = J + (res_tuples[len(Ekins1)*time][1]
...
for i in range(0,len(Ekins1) * (n-1)):
    print res_tuples[i]
